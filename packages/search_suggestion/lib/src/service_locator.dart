import 'package:get_it/get_it.dart';
import 'package:network/network.dart';
import 'package:search_suggestion/search_suggestion.dart';
import 'package:search_suggestion/src/data/search_suggestion_repository.dart';

final GetIt searchSuggestionLocator = GetIt.instance;

class SearchSuggestionServiceLocator {
  static void registerLazySingleton<T extends Object>(
    GetIt locator,
    T Function() factoryFunc,
  ) {
    if (locator.isRegistered<T>()) {
      locator.unregister<T>();
    }
    locator.registerLazySingleton<T>(factoryFunc);
  }

  static void registerFactory<T extends Object>(
    GetIt locator,
    T Function() factoryFunc,
  ) {
    if (locator.isRegistered<T>()) {
      locator.unregister<T>();
    }
    locator.registerFactory<T>(factoryFunc);
  }

  static void setup() {
    final apiClient = networkLocator<ApiClient>();

    registerLazySingleton<SearchSuggestionRepository>(
      searchSuggestionLocator,
      () => SearchSuggestionRepository(apiClient: apiClient),
    );

    registerFactory(
      searchSuggestionLocator,
      () => SearchSuggestionBloc(
        searchRepository: searchSuggestionLocator<SearchSuggestionRepository>(),
      ),
    );
  }
}
